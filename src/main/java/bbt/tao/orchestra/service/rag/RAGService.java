...
public class RAGService {
    ...
    private volatile List<Document> cachedParentDocuments = List.of();
    private final Object parentCacheLock = new Object();
    ...
    public List<Document> searchHierarchical(String query,
        ...
        // pass#1: листья
        int pass1TopK = Math.max(topKLeaf * 3, 30);
        String leafFilter = "type == 'leaf'";
        SearchRequest pass1Request = SearchRequest.builder()
                .query(qEmbed)
                .topK(pass1TopK)
                .filterExpression(leafFilter)
                .build();
        List<Document> hyd1 = queryCache.getOrCompute(
                "leaf-pass1",
                fingerprint(pass1Request),
                () -> executeSimilaritySearch("vectorStore pass#1", pass1Request)
        );
        log.info("RAGService: pass#1 -> {} document(s)", hyd1.size());
        if (log.isDebugEnabled()) {
            log.debug("RAGService: pass#1 metadata samples={}", hyd1.stream()
        ...
        // pass#2: листья
        int pass2TopK = Math.max(topKLeaf * 3, 30);
        SearchRequest pass2Request = SearchRequest.builder()
                .query(q2)
                .topK(pass2TopK)
                .filterExpression(leafFilter)
                .build();
        List<Document> hyd2 = queryCache.getOrCompute(
                "leaf-pass2",
                fingerprint(pass2Request),
                () -> executeSimilaritySearch("vectorStore pass#2", pass2Request)
        );
        log.info("RAGService: pass#2 (PRF) -> {} document(s)", hyd2.size());
        if (log.isDebugEnabled()) {
            log.debug("RAGService: pass#2 metadata samples={}", hyd2.stream()
                    .map(d -> d.getMetadata().toString())
                    .collect(Collectors.toList()));
        }
        debugTypeStats(hyd2);
    ...
    private List<Document> executeSimilaritySearch(String stage, SearchRequest request) {
        long start = System.nanoTime();
        List<Document> raw = vectorStore.similaritySearch(request);
        List<Document> hydrated = raw.stream().map(this::hydrateFromRedis).toList();
        long elapsed = (System.nanoTime() - start) / 1_000_000;
        log.info("{} fetched {} document(s) in {} ms [queryPreview='{}']", stage, hydrated.size(), elapsed, preview(request.getQuery(), 120));
        return hydrated;
    }

    private String fingerprint(SearchRequest request) {
        String filter = request.getFilterExpression() == null ? "" : request.getFilterExpression().toString();
        return request.getQuery() + "|k=" + request.getTopK() + "|f=" + filter;
    }
    ...
    private Document resolveParent(String parentId) {
        ...
        List<Document> parents = loadParentDocuments();
    ...
    private List<Document> loadParentDocuments() {
        List<Document> parents = cachedParentDocuments;
        if (!parents.isEmpty()) {
            return parents;
        }
        synchronized (parentCacheLock) {
            parents = cachedParentDocuments;
            if (!parents.isEmpty()) {
                return parents;
            }
            SearchRequest request = SearchRequest.builder()
                    ...
                    .build();
            parents = queryCache.getOrCompute(
                    "parent-catalog",
                    fingerprint(request),
                    () -> executeSimilaritySearch("vectorStore parent catalog", request)
            );
            cachedParentDocuments = parents;
            log.info("RAGService: cached {} parent document(s)", parents.size());
            return parents;
        }
    }
