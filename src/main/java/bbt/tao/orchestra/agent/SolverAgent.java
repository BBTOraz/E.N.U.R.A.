...
public class SolverAgent {
    ...
    public SolverAgent(AgentChatClientRegistry chatClientRegistry, EmbeddingToolClassifier classifier) {
    ...
    public Mono<SolverResult> solve(AgentRequestContext context, AgentScratchpad scratchpad, List<Document> ragDocuments, AgentEventPublisher publisher) {
    ...
    private SolverResult doSolve(AgentRequestContext context,
        ...
        boolean wasInitialized = scratchpad.isToolSelectionInitialized();

        if (!wasInitialized) {

            Optional<ToolCallback> classified = selectTool(context.userMessage());

            if (classified.isPresent()) {

                scratchpad.setSelectedTool(classified.get());

            } else {

                scratchpad.markNoToolSelected();

            }

            if (log.isDebugEnabled()) {

                log.debug("SolverAgent: tool classifier cache miss for conversation={} (selected={})",

                        context.conversationId(), classified.map(tool -> tool.getToolDefinition().name()).orElse("none"));

            }

        }

        Optional<ToolCallback> selectedTool = scratchpad.selectedTool();

        if (wasInitialized && log.isDebugEnabled()) {

            log.debug("SolverAgent: tool classifier cache hit for conversation={} (selected={})",

                    context.conversationId(), selectedTool.map(tool -> tool.getToolDefinition().name()).orElse("none"));

        }

        selectedTool.ifPresentOrElse(tool -> publisher.publish(
                        AgentEvent.of(AgentStage.TOOL_SELECTION, AgentVisibility.TRACE, "Tool classifier", "Selected tool " + tool.getToolDefinition().name())
        ...
        if (log.isDebugEnabled()) {
            log.debug("SolverAgent: conversation={} solverProvider={} tool={} docs={} promptPreview={}",
                    context.conversationId(),
                    context.solverProvider().id(),
                    selectedTool.map(tool -> tool.getToolDefinition().name()).orElse("none"),
                    summarizeDocuments(ragDocuments),
                    truncate(context.userMessage(), PROMPT_PREVIEW_LIMIT));
        }
        ...
        if (context.mode().isStreaming()) {
            draft = requestSpec.stream().content()
                    .doOnNext(chunk -> publisher.publish(AgentEvent.of(AgentStage.SOLVER_TOKEN, AgentVisibility.TRACE,
                            "Solver", chunk)))
                    .collectList()
                    .doOnSuccess(content -> log.info("SolverAgent: text received ={}", content.stream().toList()))
        ...
        if (log.isDebugEnabled()) {
            log.debug("SolverAgent: conversation={} draftLength={} draftPreview={}",
                    context.conversationId(),
                    draft == null ? 0 : draft.length(),
                    truncate(draft, PROMPT_PREVIEW_LIMIT));
        }
    ...
    private List<String> summarizeDocuments(List<Document> docs) {
        return docs.stream()
                .limit(5)
                .map(doc -> {
                    Object title = doc.getMetadata().getOrDefault("title", doc.getId());
                    Object score = doc.getMetadata().getOrDefault("distance", "?");
