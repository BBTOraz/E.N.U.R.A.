...
public class RedisAgentScratchpadStore implements AgentScratchpadStore {
    ...
    private static final Logger log = LoggerFactory.getLogger(RedisAgentScratchpadStore.class);
    ...
    public void saveDocuments(String conversationId, List<Document> documents) {
        ...
        if (CollectionUtils.isEmpty(documents)) {
            ...
            touchTtl(key);
            log.debug("ScratchpadStore: cleared RAG documents for conversation={}", conversationId);
            return;
        }
        ...
        log.debug("ScratchpadStore: stored {} RAG document(s) for conversation={}", documents.size(), conversationId);
    }
    ...
    public List<Document> loadDocuments(String conversationId) {
        ...
        if (!StringUtils.hasText(json)) {
            log.debug("ScratchpadStore: no cached RAG documents for conversation={}", conversationId);
            return List.of();
        }
        try {
            List<StoredDocument> stored = objectMapper.readValue(json, DOCUMENT_LIST_TYPE);
            log.debug("ScratchpadStore: cache hit with {} RAG document(s) for conversation={}", stored.size(), conversationId);
